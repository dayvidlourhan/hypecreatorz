...
const RoadmapStep = ({ phase, title, text, icon: Icon, accentColor, isLeft, badgeColor, textColor = "text-black", rotation, connectorBg, dotColor }: any) => {
    return (
        <div className={`flex flex-col md:flex-row ${isLeft ? 'md:flex-row' : 'md:flex-row-reverse'} items-center w-full mb-16 md:mb-32 last:mb-16 relative pl-12 md:pl-0`}>
            ...
            <div className={`hidden md:block absolute top-1/2 -translate-y-1/2 h-1 bg-black z-0 
        ${isLeft ? 'right-1/2 w-[calc(50%-180px)]' : 'left-1/2 w-[calc(50%-180px)]'}`}
            ...
            <div className={`w-full md:w-1/2 flex justify-center ${isLeft ? 'md:justify-end md:pr-24' : 'md:justify-start md:pl-24'}`}>
                <div className={`max-w-md w-full p-6 md:p-8 border-4 border-black bg-white shadow-brutal transition-all hover:translate-x-1 hover:translate-y-1 hover:shadow-none group relative ${rotation} md:${rotation} z-10`}>
    ...
    return (
        <section className="relative py-16 md:py-32 px-4 bg-bone overflow-hidden min-h-screen">
            ...
            <div className="max-w-6xl mx-auto text-center mb-24 md:mb-40 relative z-10 px-2">
                ...
                <p className="font-space text-lg md:text-4xl uppercase font-black tracking-tight max-w-2xl mx-auto leading-none mt-8 md:mt-12 underline decoration-electric-purple decoration-4">
            ...
            <div className="max-w-7xl mx-auto relative px-2 md:px-4">
                ...
                {/* Central Path (THE BACKBONE) - Visible behind everything */}
                <div className="absolute left-[20px] md:left-1/2 -translate-x-1/2 top-0 bottom-24 w-1 border-l-4 md:border-l-8 border-black border-dashed z-0" />
